...
001	SGH-1 05-12-2019 [Nishit Khakhi]	Added Initial Version to List Generator
002	SGH-1 16-12-2019 [Nishit Khakhi]	Added isParent Flag to Generator list for getting parent Generator data
003	SGH-1 20-12-2019 [Nishit Khakhi]	Updated Parent flag condition to give only child data when it pass as zero
004	SGH-1 30-12-2019 [Nishit Khakhi]	Added Location Name, Count of Generators
*******************************************************************/
CREATE PROCEDURE [dbo].[Generator_List]
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param =
        (
            SELECT 'Generator_List' AS '@procName'
    ...
    BEGIN TRY
		...
		IF OBJECT_ID('tempdb..#temp_Generator') IS NOT NULL DROP TABLE #temp_Generator

		CREATE TABLE #temp_Generator
		...
		( SELECT
			...
			, D.[isActive]
			, 0 AS [count]						
			FROM [dbo].[Generator] D WITH (NOLOCK) 
			INNER JOIN [dbo].[Location] G WITH (NOLOCK) ON D.[locationGuid] = G.[guid]
			 WHERE D.[companyGuid]=@companyGuid AND D.[isDeleted]=0 '
			+ CASE WHEN @isParent = 0 THEN ' AND parentGensetGuid IS NOT NULL ' ELSE
			' AND parentGensetGuid IS NULL ' END +
		...
		BEGIN
			;WITH CTEDATA
			AS 
			(	SELECT d.[parentGensetGuid],COUNT(*) [totalGenerator] FROM [Generator] D
				INNER JOIN #temp_Generator t ON D.[parentGensetGuid] = t.[Guid]
				GROUP BY d.[parentGensetGuid]
			)
			UPDATE t
			SET [count] = c.[totalGenerator]
			FROM #temp_Generator t INNER JOIN CTEDATA c ON t.[guid] = c.[parentGensetGuid]
