...
001	SG-18	14-02-2020	[Nishit Khakhi]	Added Initial Version to Add Generator File
*******************************************************************/

CREATE PROCEDURE [dbo].[GeneratorFiles_Add]
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param =
        (
            SELECT 'GeneratorFiles_Add' AS '@procName'
	...
	IF NOT EXISTS (SELECT TOP 1 1 FROM dbo.[Generator] WHERE [guid] = @generatorGuid AND [isDeleted] = 0)
	BEGIN
		SET @output = -3
		SET @fieldName = 'GeneratorNotExists'
	END
	...
	--select * from #files
	BEGIN TRAN
		INSERT INTO [dbo].[GeneratorFiles]
	           ([guid]
	           ,[generatorGuid]
	           ,[filePath]
			   ,[description]
