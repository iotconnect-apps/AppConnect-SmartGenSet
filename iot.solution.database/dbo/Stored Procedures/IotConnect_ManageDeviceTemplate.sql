	...
	FROM @DeviceTemplateXml.nodes('/items/item') a(b)
		...
		BEGIN TRAN
			...
			BEGIN
				...
				(
					SELECT [name] FROM [dbo].[KitType] k (NOLOCK)
				)

				INSERT INTO [dbo].[KitType] ([guid], [code], [name], [isDeleted], [tag], [isActive])
				SELECT DISTINCT NEWID(), [code], [name], [isDeleted], LOWER([tag]), 1
				FROM #tempDeviceTemplate te
				WHERE NOT EXISTS ( SELECT 1 FROM ExistingDeviceTemplate ee WHERE te.[name] = ee.[name])
			END
