...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			(
				SELECT [guid] FROM dbo.[Generator] d (NOLOCK) WHERE d.companyGuid = @companyGuid
			)
	
			INSERT INTO dbo.[Generator]([guid], [uniqueId], [name], [companyGuid], [locationGuid], [isActive], [isDeleted], [templateGuid], [isProvisioned], [tag], [parentGensetGuid],[createdDate],[note])
		...
		BEGIN
			UPDATE d
				SET  [uniqueId]			= CASE WHEN td.hasUniqueId = 1 THEN td.uniqueId ELSE d.[uniqueId] END
					,[locationGuid]	= CASE WHEN td.hasentityGuid = 1 THEN td.entityGuid ELSE d.locationGuid END
