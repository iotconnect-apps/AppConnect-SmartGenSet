...
001	sgh-1 05-12-2019 [Nishit Khakhi]	Added Initial Version to Delete Location
*******************************************************************/
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'Location_Delete' AS '@procName'             
    ...
    BEGIN TRY            
		...
		IF NOT EXISTS (SELECT TOP 1 1 FROM [dbo].[Location] (NOLOCK) WHERE [companyGuid] = @companyguid AND [guid] = @guid AND [isdeleted] = 0)
		BEGIN
			SET @output = -3
