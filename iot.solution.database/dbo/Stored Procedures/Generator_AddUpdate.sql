...
EXEC [dbo].[Generator_AddUpdate]
	@companyGuid			= '895019CF-1D3E-420C-828F-8971253E5784'
	,@locationGuid			= '5330D171-DA1E-4554-A868-87D222CD5D35'
	,@templateGuid			= '12A5CD86-F6C6-455F-B27A-EFE587ED410D'
	,@parentGensetGuid		= '12A5CD86-F6C6-455F-B27A-EFE587ED410D'
	,@typeGuid					= 1
...
001	sgh-1 05-12-2019 [Nishit Khakhi]	Added Initial Version to Add Update Generator
*******************************************************************/

CREATE PROCEDURE [dbo].[Generator_AddUpdate]
(	@guid				UNIQUEIDENTIFIER
	,@companyGuid		UNIQUEIDENTIFIER
	,@locationGuid		UNIQUEIDENTIFIER
	,@parentGensetGuid	UNIQUEIDENTIFIER	= NULL
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param =
        (
            SELECT 'Generator_AddUpdate' AS '@procName'
	...
	BEGIN TRY
		...
		SET @newid = @guid
		BEGIN TRAN
			IF NOT EXISTS(SELECT TOP 1 1 FROM [dbo].[Generator] (NOLOCK) where [guid] = @guid AND companyGuid = @companyGuid AND isdeleted = 0)
			BEGIN	
				INSERT INTO [dbo].[Generator]
			           ([guid]
			           ,[companyGuid]
			           ,[locationGuid]
			           ,[templateGuid]
			...
			ELSE
			BEGIN
				UPDATE [dbo].[Generator]
				SET	[locationGuid] = @locationGuid
					,[parentGensetGuid] = @parentGensetGuid
