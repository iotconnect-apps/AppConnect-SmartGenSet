...
*******************************************************************/
CREATE PROCEDURE [dbo].[Location_AddUpdate]
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'Location_AddUpdate' AS '@procName'             
	...
	BEGIN TRY
		...
		IF NOT EXISTS(SELECT TOP 1 1 FROM [Location] where [guid] = @guid and companyGuid = @companyGuid and isdeleted = 0)
		BEGIN
			IF EXISTS (SELECT TOP 1 1 FROM [Location] (NOLOCK) WHERE companyguid = @companyguid AND [isdeleted]=0 AND [name]=@name)
			BEGIN
				SET @output = -3
				SET @fieldname = 'LocationNameAlreadyExists'		 
				RETURN;
			END
		END		
