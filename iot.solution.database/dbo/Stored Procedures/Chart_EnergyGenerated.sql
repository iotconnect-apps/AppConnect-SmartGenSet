...
DECLARE @count INT
	...
	,@syncDate	DATETIME
EXEC [dbo].[Chart_EnergyGenerated]	
...
001	SG-18 16-03-2020 [Nishit Khakhi]	Added Initial Version to represent Energy Generated

*******************************************************************/
CREATE PROCEDURE [dbo].[Chart_EnergyGenerated]
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'Chart_EnergyGenerated' AS '@procName' 
    ...
    BEGIN TRY            
		...
		BEGIN
			...
			WHERE [gensetGuid] = @guid ANd ([date] BETWEEN @startDate AND @endDate) AND [attribute] = 'currentout'
		...
		BEGIN
			...
			WHERE H.[locationGuid] = @entityguid ANd ([date] BETWEEN @startDate AND @endDate) AND [attribute] = 'currentout'
			GROUP BY DATEPART(YY,[date]),DATEPART(MM,[date]),[attribute]
		END
