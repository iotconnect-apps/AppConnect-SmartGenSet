...
export class JwtInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (currentUser && currentUser.access_token) {
      ...
      var generatorGuid = res[res.length - 1];
      var image_url_genrater = request.url.includes("api/generator/manage");
      var media_url_genrater = request.url.includes("api/generator/fileupload/" + generatorGuid);
      if (image_url_genrater || media_url_genrater) {
